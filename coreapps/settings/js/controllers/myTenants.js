app.controller('viewMyTenantsCtrl', function ($scope,$rootScope,$stateParams ,$mdDialog,$http, uiInitilize, $v6urls, $auth, notifications) {
	
	console.log('start up view my tenant');
	$auth.checkSession();
	...
	{ 
		$rootScope.selectedPage = $stateParams.pageNo;
	}
...
app.controller('AddCtrl', function ($scope, $mdDialog, $window, $mdToast, notifications,$charge,$http,$rootScope, objectFormat, $mdMedia , $state) {
	...
	$scope.selectPlan = function(ev, index, package) //This is the click event for adding a company tenant in add.html
	{
		...
		tenantNameAvailability(function(data){
			...
			{
				notifications.finishLoading();
				notifications.toast("Sorry, This Tenant name is already taken","error");
			}
	...
	{
		location.href = "#/mainViewMyTenant/registration";
		$scope.showPricing = false;			
	}
	...
	$scope.submit = function(ev)
	{
		if($scope.editForm.$valid === true) 
		{
			if($scope.tenant.type && $scope.tenant.accessLevel && $scope.tenant.businessModel)//make sure all data is entered
			{
				...
				{
					location.href = "#/mainViewMyTenant/add";
	...
	$scope.goBack = function(state, pageNo)
	{
