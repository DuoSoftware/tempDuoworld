	...
	class Usergroup {
		...
		private function scanGroupsAndApps($isReportMode,$userName){
			...
			foreach ($allApps as $app){
				...
				if (strcmp(strtolower($share["id"]),strtolower($userName)) == 0){
					if ($isReportMode){
						...
						if (strcmp(strtolower($app["shares"][$i]["id"]),strtolower($userName))==0){
						$index =$i;
						break;
					}
						if(isset($index)){
			...
			foreach ($allGroups as $group){
				...
				if (strcmp(strtolower($gUser),strtolower($userName)) == 0){
					if ($isReportMode){
						$response->success = false;
						...
						if (strcmp(strtolower($group["users"][$i]),strtolower($userName))==0){
							$index =$i;
							break;
						}
