...
class CustomPlan {
    ...
    public function subscribe($plan, $token, $alacarts) {
        ...
        */
        ...
        $customplan->attributes = $this->buildAttributes($plan, $alacarts);
    ...
    public function upgrade() {
        ...
        */
        ...
        $response = (new CloudCharge())->plan()->upgradeToCustomplan($customplan);
        return $response;  
    ...
    public function customize() {
        ...
        */
        ...
        $response = (new CloudCharge())->plan()->customize("");
        return $response; 
    ...
    private function buildAttributes($plan, $alacarts, $withoutPackageAttr = false) {

        $albucket = array();

        if(!empty($alacarts)) {

            $alobjects = $this->getAlacartObject($alacarts);

            foreach ($alobjects as $alobj) {
                foreach ($alacarts as $alacart) {
                    if($alobj['code'] === $alacart->acode) {
                        if(in_array($plan->code, $alobj['validplans'])) {
                            $al = (new CustomPlanAttrirutesStruct($alobj['tag'], $alobj['name'], $alobj['price'], $alacart->quantity, $alacart->action));
                            array_push($albucket, $al);
                        } 
                    }
                }
            }
    ...
    private function getAlacartObject($alacarts) {
        $alcodes = array();
        foreach ($alacarts as $alacart)
            array_push($alcodes, $alacart->acode);

        return (new Alacart())->get($alcodes);
...
class Alacart {
    ...
    public function get($al) {

        $alcodestr = "";
        if(is_array($al)) {
            $alcodestr = implode("','", $al); $alcodestr = "'$alcodestr'";
        }else $alcodestr = "'$al'";

        $client = ObjectStoreClient::WithNamespace("com.duosoftware.com", "priceplan_alacarts", "123");
        $osresponse = $client->get()->byFiltering("SELECT * FROM student where code IN ($alcodestr)");

        if(isset($osresponse->IsSuccess) && !$osresponse->IsSuccess) 
            throw new Exception("Data fetching error: $osresponse->Message");
...
class PricePlan {
    ...
    public function subscribe($p) {
        ...
        if($plan->price === 0.0) { // free plan or trail plan
            ...
            if(!isset($p->token)) // token not available, token required.
            ...
            if($plan->price > 0.0 && !$plan->customizable) {// fixed plan
                ...
                $planClient = new CustomPlan();
                $ccresponse = $planClient->subscribe($plan, $p->token, (isset($p->alacarts)) ? $p->alacarts : array());
            }
    ...
    public function checkSubscription() {
        ...
        $cplan = $this->_currentPlan();
        if(empty($cplan)) throw new Exception("Tenant Subscription: Not subscribed to any plan.");
        ...
        $remaining = json_encode($this->getRemainingDays($cplan->currentperiodend));
        echo "{\"success\": $subscriptionStatus, \"data\": {\"remaining\": $remaining}}";
        return;
    ...
    public function stopSubscription($immediately) {

        (new CloudCharge())->plan()->stopSubscription($immediately);
    ...
    private function getRemainingDays($currentperiodend) {
        $remaining = $currentperiodend - time();
        $days_remaining = floor($remaining / 86400);
        $hours_remaining = floor(($remaining % 86400) / 3600);

        return json_decode("{\"days\": $days_remaining, \"hours\": $hours_remaining}");
...
class PlanService {
    ...
    public function info() {
        $info = new stdClass();
        $info->name = "Price plan service";
        $info->version = "1.0.9";
        $info->changeLog = "added new rest endpoint, for the purpose of subscription stop. endpoint: apis/plan/subscription/stop(/now)";
    ...
    public function subscribe() { 
        ...
        try {
            ...
            echo '{"success": false, "message": "'.$ex->getMessage().$ex->getLine().'"}';
        }
    ...
    public function upgrade() { 
        ...
        try {
            ...
            echo '{"success": false, "message": "'.$ex->getMessage().'"}';
        }
    ...
    public function stopSubscription($now) {
        ...
        try {
            ...
            if(is_null($now)) $now = false;
            else {
                if($now === 'true') $now = true; 
                else $now = false;
            }
           
            (new PricePlan())->stopSubscription($now);
        } catch(Exception $ex) {
            echo '{"success": false, "message": "'.$ex->getMessage().'"}';
        }
    ...
    function __construct() {
        ...
        // GET /plan/
        Flight::route("GET /" , function() { $this->info(); });
        ...
        Flight::route('GET /subscription/stop(/@stopnow)', function($stopnow) { $this->stopSubscription($stopnow); });

        // GET /plan/subscription/reactive
        Flight::route('GET /subscription/reactive', function() { /* subscription reactivation */});
